#!/usr/bin/env python3
"""Maintain one hold/smoke issue per label. Used only by the publishing workflow."""
import argparse
import json
import os
...
def main():
  ...
  issues = json.loads(gh("issue", "list", "--state", "open", "--label", label, "--limit", "1000", "--json", "number"))
  ...
  if args.close:
    for number in numbers:
      gh("issue", "close", str(number), "--comment", f"Nightly succeeded for upstream {args.upstream}.\n\n{run_url}")
    return
  ...
  title = f"nightly {args.kind}: {reason}"[:200]
  ...
  with tempfile.TemporaryDirectory(prefix="nightly-issue-") as directory:
    ...
    if numbers:
      ...
      gh("issue", "create", "--title", title, "--label", label, "--body-file", str(body_file))
